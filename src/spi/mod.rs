    ...
    SPI: embedded_hal_async::spi::SpiDevice,
{
    ...
    pub async fn read<'a>(
        ...
        self.spi
            .transaction(&mut [Operation::Write(&[cmd]), Operation::Read(buf)]).await?;
        Ok(buf)
    }

    pub async fn read_single(&mut self, reg: Register) -> Result<u8, SPI::Error> {
        let mut buf = [0];
        self.read(reg, &mut buf).await?;
        Ok(buf[0])
    }

    pub async fn write(&mut self, reg: Register, buf: &[u8]) -> Result<(), SPI::Error> {
        let mut cmd = reg as u8 & 0x7F; // Clear the read bit
        if buf.len() > 1 {
            cmd |= 0x40; // Set multi-byte write bit
        }
        self.spi
            .transaction(&mut [Operation::Write(&[cmd]), Operation::Write(buf)]).await?;

        Ok(())
    }

    pub async fn set_data_rate(&mut self, rate: DataRate) -> Result<(), SPI::Error> {
    ...
    pub async fn set_fifo_mode(&mut self, mode: FifoMode) -> Result<(), SPI::Error> {
    ...
    pub async fn read_fifo(&mut self) -> Result<heapless::Vec<u8, FIFO_SIZE_BYTES>, SPI::Error> {
        ...
        for sample in buf.chunks_exact_mut(6) {
            ...
            self.spi.transaction(&mut [Operation::DelayNs(5_000)]).await?; 
        }
